...
const BottomNav: React.FC<BottomNavProps> = ({ activeView, onChangeView }) => {
  ...
  return (
    <div className="fixed bottom-6 left-1/2 -translate-x-1/2 z-40 w-auto">
      <div className="flex items-center gap-2 glass backdrop-blur-xl border border-white/10 shadow-2xl shadow-indigo-500/10 rounded-2xl px-4 py-3">
        {navItems.map((item) => {
            ...
            <button
              ...
              className={`relative flex items-center justify-center gap-3 min-h-[60px] min-w-[100px] px-4 rounded-xl transition-all duration-300 border ${
                isActive
                  ...
                  : "text-indigo-200 hover:text-white glass border-white/5 hover:border-indigo-500/30 focus:ring-2 focus:ring-indigo-500/50"
              }`}
            >
